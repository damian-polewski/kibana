...
/**
 * `logsSynthMappings()` maps exactly 25 fields — leaf fields, the objects holding
 * them and multi-fields all count towards `mapping.total_fields.limit` — so the
 * limits below are picked relative to that count:
 *  - 25 leaves no room at all, so the whole `cloud` object added by the second
 *    ingest is rejected and reported as the `cloud` quality issue;
 *  - 26 (set on the backing index the rollover creates) leaves room for `cloud`
 *    alone, so the third ingest is reported one level deeper, as `cloud.project`.
 */
 ...
 *
 * Kept local because these specs needed a helper before the shared page object modelled
 * this URL state. `pageObjects.datasetQualityDetails.goto` now supports
 * `expandedQualityIssue`, `showCurrentQualityIssues`, `qualityIssuesChart`, `view` and
 * `breakdownField` — collapsing these helpers onto it is a worthwhile follow-up, it just
 * needs the specs re-verified against both deployments.
 */
...
test.describe(
  ...
  () => {
    ...
    // Deleting the data stream also drops the extra backing index and the field
    // limit settings, neither of which the FTR suite ever reverted.
    ...
    test('closes the flyout for an issue missing from the latest backing index', async ({
      ...
      await test.step('when the URL expands a past issue and asks for current ones', async () => {
        ...
        // Wait for the toggle to reflect the URL state before asserting on the
        // absence of the flyout, otherwise the assertion could pass too early.
        await expect
          .poll(async () => datasetQualityDetails.isCurrentQualityIssuesToggleChecked())
          .toBe(true);
