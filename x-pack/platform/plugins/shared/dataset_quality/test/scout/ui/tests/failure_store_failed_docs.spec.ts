...
/**
 * The scenarios that need documents to actually land in the failure store. The panels,
 * modal and table link, which only need the failure store enabled or disabled, live in
 * failure_store.spec.ts.
 *
 * Both of these assert counts, so they depend on `refreshFailureStore`: the synthtrace
 * client does not refresh the `.fs-*` indices, which previously made these assertions
 * flaky — reliably on serverless, intermittently on stateful.
 */
