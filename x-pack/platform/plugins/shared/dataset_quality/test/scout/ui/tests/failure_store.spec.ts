...
test.describe(
  ...
  () => {
    ...
    test.afterAll(async ({ esClient, log, logsSynthtraceEsClient }) => {
      ...
      for (const name of [DISABLED_DATA_STREAM, TABLE_DATA_STREAM]) {
        await esClient.indices.putDataStreamOptions(
          { name, failure_store: { enabled: false } },
          { meta: true }
        );
      }

      await logsSynthtraceEsClient.clean();
      for (const name of [ENABLED_DATA_STREAM, DISABLED_DATA_STREAM, TABLE_DATA_STREAM]) {
        await deleteDataStreamIfExists(esClient, name, log);
      }
      await deleteIndexTemplateIfExists(esClient, ENABLED_INDEX_TEMPLATE, log);
      await deleteIndexTemplateIfExists(esClient, DISABLED_INDEX_TEMPLATE, log);
    ...
    test('offers "Set failure store" from the table and removes the link once enabled', async ({
      ...
      const setFailureStoreLink = page.testSubj.locator(
        `datasetQualitySetFailureStoreLink-${TABLE_DATA_STREAM}`
      );
