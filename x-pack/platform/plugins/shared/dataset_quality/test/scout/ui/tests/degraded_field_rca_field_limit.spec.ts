...
/**
 * Every field limit below is derived from the field count of the mapping fixture the
 * data stream is pinned to. Leaf fields, the objects holding them and multi-fields
 * all count towards `mapping.total_fields.limit`.
 *
 * The limit set on the first backing index equals (or undercuts) the mapped field
 * count, so the whole `cloud` object of the second ingest is rejected and reported as
 * the `cloud` quality issue. The limit set on the backing index created by the
 * rollover leaves room for `cloud`, so the third ingest is reported one level deeper:
 * as `cloud.project` where only one slot is left, and as `cloud.project.id` for nginx
 * where `cloud` is already mapped by the fixture.
 *
 * The stateful and serverless FTR suites both set 42/43 for nginx and asserted 43 in
 * the UI; only their comments disagreed (one called the second ingest "44 fields").
 */
 ...
 *
 * Kept local because these specs pass URL state the shared page object's `goto` does
 * not model (and needed a helper before it did). `pageObjects.datasetQualityDetails.goto`
 * now supports `expandedQualityIssue`, `showCurrentQualityIssues`, `qualityIssuesChart`,
 * `view` and `breakdownField` — collapsing these four helpers onto it is a worthwhile
 * follow-up, it just needs the specs re-verified against both deployments.
 */
...
test.describe(
  ...
  () => {
    ...
    // Deleting the data streams also drops the extra backing indices and the field
    // limit settings, neither of which the FTR suite ever reverted. The nginx
    // `@custom` component template is removed too: the last test edits it, and the
    // FTR suite left that edit behind for every later run.
    // One chain per data set, run concurrently so a failure in one still tears the
    // others down. Each chain stays ordered internally: a component template cannot be
    // deleted while an index template — or an installed package's template — composes it.
