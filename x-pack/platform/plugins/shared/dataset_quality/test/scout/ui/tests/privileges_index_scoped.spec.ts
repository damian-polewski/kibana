...
/**
 * Data sets owned by this spec. `plain` is never monitorable; `monitored` is the
 * one the mixed role below grants `monitor` on, which is how the FTR suite
 * contrasted a data stream with and without the privilege.
 */
...
// serverless project.
test.describe('Dataset quality privileges - index scoped', { tag: tags.stateful.classic }, () => {
  // Read-only data seeded once with the privileged synthtrace client: each scenario
  // varies the role, never the documents.
  ...
  test('warns per data set when only some can be monitored', async ({
    ...
    await test.step('badges only the data set without the monitor privilege', async () => {
      ...
      await expect(
        pageObjects.datasetQuality.getInsufficientPrivilegesBadge(`sizeBytes-${MONITORED_DATASET}`)
      ).toBeHidden();
    });
  ...
  test('hides "View dashboards" from a user without dashboard access', async ({
    ...
    await expect(pageObjects.datasetQualityDetails.getIntegrationAction('Overview')).toBeVisible();
    await expect(pageObjects.datasetQualityDetails.getIntegrationAction('Template')).toBeVisible();
    await expect(
