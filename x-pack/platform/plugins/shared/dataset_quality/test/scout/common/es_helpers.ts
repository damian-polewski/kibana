...
/**
 * Makes documents that were rejected into a data stream's failure store searchable.
 *
 * Call this after ingesting rejected documents and before asserting on them. The
 * synthtrace client's `refreshAfterIndex` only refreshes the `logs-*-*` data streams,
 * not the `.fs-*` indices behind `<data stream>::failures`, so failed documents stay
 * invisible to search until Elasticsearch refreshes on its own schedule. That wait is
 * timing-dependent — and noticeably slower on serverless, where search reads from the
 * object store — which made every assertion on failed-document counts flaky.
 *
 * A missing failure store is not an error: specs assert the empty case too. Closed
 * backing indices are skipped rather than refreshed — some specs close one on purpose
 * to assert it is excluded, and refreshing it would raise `index_closed_exception`.
 */
...
/**
 * Runs every teardown step even if earlier ones fail, then reports all failures.
 *
 * Sequential `await`s in a teardown hook mean the first failure skips everything
 * after it, which is how the FTR suites leaked Fleet packages and templates.
 */
export const cleanUpAll = async (steps: Array<() => Promise<unknown>>): Promise<void> => {
  ...
  if (failures.length > 0) {
    throw new Error(
      `Teardown failed: ${failures.map(({ reason }) => reason?.message ?? reason).join('; ')}`
    );
  }
};
