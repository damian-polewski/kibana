...
apiTest.describe(
  ...
  () => {
    ...
    // `sizeBytes` comes from the metering API. It is asserted on both deployments
    // because it does work on serverless here — unlike the details page Size KPI in
    // the UI, which reads from `_stats` and stays at 0 there (elastic/kibana#178954).
    apiTest(
