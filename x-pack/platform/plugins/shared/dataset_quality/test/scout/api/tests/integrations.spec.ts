...
/**
 * Runs every teardown step even when an earlier one fails. The FTR suite awaited
 * its cleanup sequentially, so the first failure leaked every resource after it.
 */
  ...
  () => {
    ...
    apiTest(
      ...
      async ({ apiClient, samlAuth }) => {
        ...
        expect(getIntegrationNames(integrations)).toStrictEqual(
          [...preExistingIntegrationNames, ...REGISTRY_PACKAGES, CUSTOM_INTEGRATION_NAME].sort()
        );
