...
apiTest.describe(
  ...
  () => {
    ...
    apiTest(
      'skips failed documents held in a closed failure store index',
    ...
    apiTest(
      'reports failed documents again once a new failure store backing index is open',
      async ({ apiClient, esClient, logsSynthtraceEsClient }) => {
        apiTest.setTimeout(120_000);
        ...
        // Only the document in the open backing index is counted; the one in the
        // closed index stays invisible.
        await expect
          .poll(
            async () => {
              const response = await apiClient.get(failedDocsUrl(), {
